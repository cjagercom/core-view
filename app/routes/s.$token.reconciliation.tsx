    ...
    <div className="min-h-dvh bg-paper">
      <div className="w-full max-w-[480px] mx-auto px-5 flex flex-col" style={{ height: '100dvh' }}>
        ...
        <div className="flex items-center gap-3 py-4 border-b border-paper-dark">
          ...
          <div>
            <h1 className="font-display text-lg text-ink">Feedback reconciliation</h1>
            <p className="text-xs text-ink-muted">Exploring the gaps between self and others</p>
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          {!completed ? (
            ...
            >
              <DeepDiveChat sessionId={token} endpoint="reconciliation" onComplete={handleComplete} />
            </motion.div>
          ) : (
            <motion.div
              key="done"
            ...
            >
              ...
              <div className="text-center">
                <h2 className="font-display text-xl text-ink mb-2">Profile reconciled</h2>
                <p className="text-sm text-ink-soft max-w-[300px]">
                  Your profile now integrates both your self-perception and how others see you.
                </p>
                {result?.integrated_score_rationale && (
                  <div className="mt-4 text-left text-xs text-ink-muted space-y-1">
                    {Object.entries(result.integrated_score_rationale).map(([dim, text]) => (
                      <p key={dim}>
                        <span className="font-medium capitalize">{dim}:</span> {text}
                      </p>
                    ))}
                  </div>
                )}
              </div>
              <Button onClick={() => navigate(`/s/${token}`, { replace: true })}>View reconciled profile</Button>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
